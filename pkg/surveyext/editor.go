...
{{- else }}
  ...
  {{- if and .Default (not .HideDefault)}}{{color "white"}}({{.Default}}) {{color "reset"}}{{end}}
	{{- color "cyan"}}[(e) to launch {{ .EditorCommand }}{{- if .SkipEditorAllowed }}, enter to skip{{ end }}] {{color "reset"}}
{{- end}}`
...
func (e *GhEditor) prompt(initialValue string, config *survey.PromptConfig) (interface{}, error) {
	err := e.Render(
		...
		// EXTENDED to support printing editor in prompt and SkipEditorAllowed
		EditorTemplateData{
			Editor:            *e.Editor,
			SkipEditorAllowed: e.SkipEditorAllowed,
	...
	for {
		...
		if string(r) == config.HelpInput && e.Help != "" {
			err = e.Render(
				EditorQuestionTemplate,
				EditorTemplateData{
					// EXTENDED to support printing editor in prompt, SkipEditorAllowed
					Editor:            *e.Editor,
					SkipEditorAllowed: e.SkipEditorAllowed,
					EditorCommand:     filepath.Base(e.editorCommand()),
					ShowHelp:          true,
